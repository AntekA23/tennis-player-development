...
// Run with: npx tsx get-test-ids.ts

import { db } from './src/db';
import { calendarEvents, users, eventParticipants } from './src/db/schema';
...
async function getTestIds() {
  ...
  const trainingEvents = await db
    .select({
      ...
      participants: sql<number>`COUNT(DISTINCT ${eventParticipants.user_id})::int`
    })
    .from(calendarEvents)
